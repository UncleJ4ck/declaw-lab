...
# cert, reads the plaintext request, and hands it onward. Every request line is also
# logged to capture/traffic.log.
#
...
def main():
    ...
    LOG = os.path.join(ROOT, "capture", "traffic.log")
    ...
    if not (os.path.exists(CERT) and os.path.exists(KEY)):
        subprocess.run(
            ["openssl", "req", "-x509", "-newkey", "rsa:2048", "-nodes",
             ...
             "-subj", "/CN=declaw-mitm-not-a-real-ca"],
            check=True,
        )
    ...
    sctx.set_alpn_protocols(["http/1.1"])          # force HTTP/1.1 so requests are readable
